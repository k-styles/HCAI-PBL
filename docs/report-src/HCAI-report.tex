...
\file{data/}       & \file{iris.csv} and \file{penguins.csv}, used during development. \\
...
end to end, chooses the penalty, reports the error, and closes the slide with
``Conclusion: no human needed!'' --- immediately followed by a slide
...
distinction is small on the datasets used here --- a little under 1{,}100 units of
MSE on the body-mass sweep --- and large in principle, and it is stated on the
training page so the user knows which of the two they are reading.
...
\paragraph{Penguin body mass, ridge regression, mean squared error.} A
regression problem, and the case where the rule actually overrules the peak.
Predicting body mass in grams from bill length, bill depth and flipper length,
the best mean sits at $\alpha=1$, but $\alpha=10$ lies within one standard error
of it, and the more strongly penalised model is the one kept. On the held-out
rows it scores \emph{better} --- 145{,}559 against 146{,}617 --- which is luck
rather than proof, but it is emphatically not the trade going wrong.
...
\midrule
0.001 & 165{,}102.9 & 15{,}754.9 & 154{,}196.4 & tied \\
0.01  & 165{,}101.0 & 15{,}754.3 & 154{,}196.4 & tied \\
0.1   & 165{,}082.9 & 15{,}747.8 & 154{,}196.7 & tied \\
1     & 164{,}935.1 & 15{,}692.5 & 154{,}228.6 & best mean \\
10    & 165{,}846.9 & 15{,}696.7 & 156{,}552.1 & \textbf{kept} \\
100   & 208{,}257.0 & 18{,}379.0 & 202{,}886.7 & \\
1000  & 433{,}894.0 & 20{,}785.4 & 427{,}921.4 & \\
\bottomrule
\end{tabular}
\caption{Five-fold sweep over the ridge penalty, predicting penguin body mass
from three body measurements. 256 training rows, 86 held back. The standard
error is around 15{,}700 throughout, which swamps the 912-unit gap between the
best mean and the value actually kept --- so the two are not distinguishable on
this sample, and the rule is entitled to prefer the plainer one. Note also how
little the first four rows differ: across three orders of magnitude of $\alpha$
the fit barely moves.}
\label{tab:penguin-mass}
\end{table}

\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{figures/penguin-mass-sweep.png}
\caption{The same sweep scored by $R^2$. The curve is almost flat across four
orders of magnitude of $\alpha$ before it collapses, which is the honest shape
of this problem and is invisible if a grid search returns only its argmax. The
dashed line is the score on the rows each model was fitted on; the gap between
the two is small here, which is what a well-conditioned regression looks like.}
\label{fig:penguin-mass-sweep}
\end{figure}
